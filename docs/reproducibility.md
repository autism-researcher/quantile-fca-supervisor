...
The canonical paper figures are regenerated by
`analysis/make_all_figures.py`, which reads the per-controller summary
CSVs bundled in this repository under `analysis/data/analysis_outputs/`.
Run it from inside the `analysis/` folder so the relative data paths
resolve correctly:
...
This regenerates the paper's Pareto, calibration, forest, heatmap, and
comfort figures (Figures 3–8) as `fig_*.png` / `fig_*.pdf` in `analysis/`.

The legacy scripts `analysis/regenerate_pareto.py` and
`analysis/cross_seed_figure.py` are retained for secondary checks only;
they are **not** the canonical paper-figure path.
...
This step is only required if we want to verify the simulation pipeline
...
Repeat for seeds 2000 and 3000 by changing `--seed0`. Each replication
produces 600 episodes:
240 Stage A calibration + 360 Stage B evaluation, with each Stage B
episode re-run under all 8 controllers.

## Expected results

After running the analysis pipeline we should reproduce, to within
...
| H2 p-value (P_{0.20} vs T_{3.5}) | 6.0×10⁻⁷ |
...
If we obtain different values, please open a GitHub issue with a
description of our environment (Python version, OS, package versions
